...
<html>
	           ...
	           <tr>
				...
				<script>
					...
					$("#add").click(function(){
						layer.open({
							 ...
							 closeBtn: 1,
							 anim: 2,
							 content: "administrative_luru.php",
							  // btn: ["", "取消"],
				...
				$("#button").click(function(){
					 $('#tb_user').bootstrapTable('destroy'); //动态加载表格之前，先销毁表格
						...
						var nd = $("#nd").val();
						var yf = $("#yf").val();
						var search= "dwmc=" + dwmc + "&htmc=" + htmc + "&htbh=" + htbh + "&nd=" + nd + "&yf=" + yf ;
							$.ajax({
											...
											var   data = eval("("+msg+")"); //将返回的json数据进解析，并赋给data									
												...
												width:50,
												                        formatter: function (value, row, index) {//这里的三个参数：value表示当前行当前列的值；row表示当前行的数据；index表示当前行的索引（从0开始）。
																			...
																			'<a href="javascript:;" onclick="ShowImg(\''+row.dwmc+'\',\''+row.htmc+'\',\''+row.lxr+'\',\''+row.qdsj+'\')" title="查看图片" class="easyui-linkbutton l-btn l-btn-small l-btn-plain" ><span class="l-btn-left l-btn-icon-left"><span class="l-btn-text"></span><span class="l-btn-icon icon-search">&nbsp;</span></span></a>'+
																			'</div>';
												                            return html;
												                        },
						...
						var yf = $("#yf").val();
						 if(htbh=="" &&   dwmc=="" && htmc=="" && nd=="" && yf==""){
								layer.open({
								 ...
								 offset:'t',  //设置弹出位置
								 shadeClose: true,
								 content: 'delete_administrative.php?dwmc=' + dwmc + '&htmc=' + htmc + '&htbh=' + htbh + '&nd=' + nd + '&yf=' + yf,
								 ...
								 yes: function (index, layero) {
									 var thisIndex = index;
									   ...
									   layer.confirm('<h4 style="color:red;padding:5px5px;magin:2px;">警告：真的确定清空表中所有数据吗</h4></center>', { icon: 5, title: '提示' }, function (index) {
											window.location.href= "delete_administrative_php.php?&dwmc=" + dwmc + "&htmc=" + htmc + "&htbh=" + htbh + "&nd=" + nd + "&yf=" + yf ;										                                									                 
																																});
																},//layer嵌套  *****重要
											});
											}else{
	...
	<script>
		...
		function excel(){
			...
			var nd = $("#nd").val();
			var yf = $("#yf").val();
			window.location.href= "excel_administrative.php?&dwmc=" + dwmc + "&htmc=" + htmc + "&htbh=" + htbh + "&nd=" + nd + "&yf=" + yf + "";
							// layer.open({
							//  type: 2,
							//  skin: 'layui-layer-demo',
							//   area: ['1200px', '300px'],
	...
	<script>
		function ifram(){
			...
			var nd = $("#nd").val();
			var yf = $("#yf").val();
			window.location.href= "administrative.php?&dwmc=" + dwmc + "&htmc=" + htmc + "&htbh=" + htbh + "&nd=" + nd + "&yf=" + yf + "";
		}
	</script>
	<script>
