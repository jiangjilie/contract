...
<html>
	...
	<!-- 	<link href="css/font-awesome.min.css" rel="stylesheet"/>
		...
		<body marginwidth="0" marginheight="0">
			<table id="dg" style="word-break:break-all; word-wrap:break-all;" class="table table-striped table-bordered table-hover" cellspacing="0" width="100%">
				<thead>
				  <tr style="background-color:#438eb9;color: #FFF;">
					...
					<th class="tr-th" style="text-align:center;">收款次数</th>
					<th class="tr-th" style="text-align:center;">收款金额</th>				
					<th class="tr-th" style="text-align:center;">凭证号</th>				
					<th class="tr-th" style="text-align:center;">收款日期</th>				
				...
				//连接数据库
					...
					echo "<td  colspan='18' >表中无收款记录存在!</td>";
					// return false;					
					}	
					$i=0;
			...
			$sql2="select count(*) as rows, sum(jshj) as zysk from bill where htbh='$htbh' and jshj is not null and jshj !=0  ";
			$result2 = mysql_query($sql2,$link);
			@$row = mysql_fetch_assoc($result2);
			$rows=$row["rows"]+1;
			if($row["zysk"]!=0 && $row["zysk"]!==''){
			 $zys = $row["zysk"]."元";;
			}else{
			 $zys = 0;
			}
			echo "<tr align='center'>";
			echo "<td>累计总收款</td>";
			echo "<td colspan='6'><input  id='zysk' placeholder=''  value='$zys' type='text' readonly style='border:0'width: 100%;height: 60%;color:black;'></td>";
			echo "</tr>";
			echo "<tr align='center'>";
			...
			<form method='get' action='bill_luruphp_income.php' id='Form'>
				<?php
				...
				<tr align='center' id="one" >
					...
					<td style="border: 0;">第<input style="border:0;width: 16px;text-align: center;" type='text' name="number" value="<?php echo $rows;?>">次收款</td>
					<td style="border: 0;"><textarea  name='ysk'  style="width: 138px;height: 26px"placeholder='价税合计'></textarea></td>
					...
					<td style="border: 0;"><span class="ace-calendar-picker" >
									<div style="top: 40px;">
										<input  id="dpStart" placeholder="收款时间" name="kprq" autocomplete='off' type="text" class="ace-calendar-picker-input ace-input"style="width:100px;height: 60%;color:black">
					...
					<td style="border: 0;"><!-- <a href="javascript:;" id="add1" onclick="add1()" title="添加收款记录" class="easyui-linkbutton l-btn l-btn-small l-btn-plain" iconcls="icon-add" plain="true" group=""><span class="l-btn-left l-btn-icon-left"><span class="l-btn-text"></span><span class="l-btn-icon icon-add">&nbsp;</span></span></a> --><a href="javascript:;" id="delete1" title="点击收起" onclick="delete1()" class="easyui-linkbutton l-btn l-btn-small l-btn-plain" iconcls="icon-remove" plain="true" group="" style="display: inline-block;"><span class="l-btn-left l-btn-icon-left"><span class="l-btn-text"></span><span class="l-btn-icon icon-remove">&nbsp;</span></span></a></td>
				
				</tr>
				<tr>
				<td  colspan='6' style='text-align: center;border: 0;'><center><a href='javascript:;' id='ok' class='easyui-linkbutton l-btn l-btn-small l-btn-plain' onclick='subForm();' ><span class='l-btn-left l-btn-icon-left'><span class='l-btn-text'>添加收款记录</span><span class='l-btn-icon icon-ok'>&nbsp;</span></span></a></td>
				</tr>
				<tr align='center' id='two' >
					...
					<td style="border: 0;"><textarea  name='ysk' id='ysk' style="width: 100px;"placeholder='收款金额'></textarea></td>
...
<!-- 			<div class="box-footer layer-footer" style="float:right"> -->
				...
				<button style='display: ;'type="button" id='save'class="btn btn-primary" onclick="Save()">保存</button>
			</center>
		...
		function Save() {
			var zysk=$("#zysk").val();
			if(zysk!=''){
			$("#ysk" , window.parent.document).val(zysk.substring(0,zysk.length-1)); // 传给父窗口
			// $("#ykp" , window.parent.document).val(zysk.substring(0,zysk.length-1)); // 传给父窗口
			$("#ysk" , window.parent.document).keyup(); //触发yfk的keyup事件更新应收款
			}else{
				$("#ysk" , window.parent.document).val(0); // 传给父窗口
				// $("#ykp" , window.parent.document).val(0); // 传给父窗口
				$("#ysk" , window.parent.document).keyup(); //触发yfk的keyup事件更新应收款
			}
		...
		<!-- <script>
			function add(){
				document.getElementById('one').innerHTML="<td style='border: 0;'>第<input style='border:0;width: 16px;text-align: center;' type='text' name='number' value='<?php echo $rows;?>'>次收款</td><td style='border: 0;'><textarea  name='ysk' id='ysk' style='width: 100px;'placeholder='收款金额'></textarea></td><td style='border: 0;'><textarea  name='pzh' id='pzh' style='width: 100px;'placeholder='凭证号'></textarea></td><td style='border: 0;'><span class='ace-calendar-picker' style='width:100%;'><div><input  id='dpStart' placeholder='收款时间' name='kprq' autocomplete='off' type='text' class='ace-calendar-picker-input ace-input'style='width: 100%;height: 60%;color:black'><span class='ace-calendar-picker-icon'></span></div></span></td><td style='border: 0;'><textarea  name='bz' id='bz'style='width: 100px;'placeholder='备注'></textarea></td><td style='border: 0;'><a href='javascript:;' id='add1' onclick='add1()'     title='添加收款记录' class='easyui-linkbutton l-btn l-btn-small l-btn-plain' iconcls='icon-add' plain='true' group=''><span class='l-btn-left l-btn-icon-left'><span class='l-btn-text'></span><span class='l-btn-icon icon-add'>&nbsp;</span></span></a><a href='javascript:;' id='delete1' title='点击删除选定记录' onclick='delete1()' class='easyui-linkbutton l-btn l-btn-small l-btn-plain' iconcls='icon-remove' plain='true' group='' style='display: inline-block;'><span class='l-btn-left l-btn-icon-left'><span class='l-btn-text'></span><span class='l-btn-icon icon-remove'>&nbsp;</span></span></a></td>"
				document.getElementById('button').innerHTML="<td  colspan='6' style='text-align: center;border: 0;'><center><a href='javascript:;' id='ok' class='easyui-linkbutton l-btn l-btn-small l-btn-plain' onclick='subForm();' ><span class='l-btn-left l-btn-icon-left'><span class='l-btn-text'>保存</span><span class='l-btn-icon icon-ok'>&nbsp;</span></span></a></td>";
			}
			function add1(){
				document.getElementById('two').innerHTML="<td style='border: 0;'>第<input style='border:0;width: 16px;text-align: center;' type='text' name='number' value='<?php echo $rows+1;?>'>次收款</td><td style='border: 0;'><textarea  name='ysk' id='ysk' style='width: 100px;'placeholder='收款金额'></textarea></td><td style='border: 0;'><textarea  name='pzh' id='pzh' style='width: 100px;'placeholder='凭证号'></textarea></td><td style='border: 0;'><textarea  name='kprq' id='kprq' style='width: 100px;'placeholder='收票日期'></textarea></td><td style='border: 0;'><textarea  name='bz' id='bz'style='width: 100px;'placeholder='备注'></textarea></td><td style='border: 0;'><a href='javascript:;' id='add1' onclick='add2()'     title='添加收款记录' class='easyui-linkbutton l-btn l-btn-small l-btn-plain' iconcls='icon-add' plain='true' group=''><span class='l-btn-left l-btn-icon-left'><span class='l-btn-text'></span><span class='l-btn-icon icon-add'>&nbsp;</span></span></a><a href='javascript:;' id='delete2' title='点击删除选定记录' onclick='delete2()' class='easyui-linkbutton l-btn l-btn-small l-btn-plain' iconcls='icon-remove' plain='true' group='' style='display: inline-block;'><span class='l-btn-left l-btn-icon-left'><span class='l-btn-text'></span><span class='l-btn-icon icon-remove'>&nbsp;</span></span></a></td>";
			}
			function add2(){
				document.getElementById('three').innerHTML="<td style='border: 0;'>第<input style='border:0;width: 16px;text-align: center;' type='text' name='number' value='<?php echo $rows+2;?>'>次收款</td><td style='border: 0;'><textarea  name='ysk' id='ysk' style='width: 100px;'placeholder='收款金额'></textarea></td><td style='border: 0;'><textarea  name='pzh' id='pzh' style='width: 100px;'placeholder='凭证号'></textarea></td><td style='border: 0;'><textarea  name='kprq' id='kprq' style='width: 100px;'placeholder='收票日期'></textarea></td><td style='border: 0;'><textarea  name='bz' id='bz'style='width: 100px;'placeholder='备注'></textarea></td><td style='border: 0;'><a href='javascript:;' id='add1' onclick='add3()'     title='添加收款记录' class='easyui-linkbutton l-btn l-btn-small l-btn-plain' iconcls='icon-add' plain='true' group=''><span class='l-btn-left l-btn-icon-left'><span class='l-btn-text'></span><span class='l-btn-icon icon-add'>&nbsp;</span></span></a><a href='javascript:;' id='delete3' title='点击删除选定记录' onclick='delete3()' class='easyui-linkbutton l-btn l-btn-small l-btn-plain' iconcls='icon-remove' plain='true' group='' style='display: inline-block;'><span class='l-btn-left l-btn-icon-left'><span class='l-btn-text'></span><span class='l-btn-icon icon-remove'>&nbsp;</span></span></a></td>";
			}
			// function delete0(){
			// 	document.getElementById('table').style.display="none";
			// }
		...
		<script>
			function ShowEdit(id){
				var iid = id;
				 window.location.href="bill_change_income.php?user=<?php echo $user;?>&id=" + iid + "";
				}
		</script>
		<script>
				function ShowDel(id) {					
						layer.open({
						 ...
						 yes: function () {
						                     var iid = id;
						                     var jsy = $("#jsy").val();
						                     window.location.href="bill_delete_income.php?user=<?php echo $user;?>&id=" + iid + "";
						                 },
						});
					}		
			</script>
