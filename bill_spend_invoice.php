	...
	<!-- 	<link href="css/font-awesome.min.css" rel="stylesheet"/>
		...
		<body marginwidth="0" marginheight="0">
			<table id="dg" style="word-break:break-all; word-wrap:break-all;" class="table table-striped table-bordered table-hover" cellspacing="0" width="100%">
				<thead>
				  <tr style="background-color:#438eb9;color: #FFF;">
					...
					<th class="tr-th" style="text-align:center;">收票次数</th>
					<th class="tr-th" style="text-align:center;">收票金额</th>
					<th class="tr-th" style="text-align:center;">税额</th>
					<th class="tr-th" style="text-align:center;">凭证号</th>
					<th class="tr-th" style="text-align:center;">发票时间</th>	
				...
				//连接数据库
					...
					$sql = "select * from 	bill where htbh='$htbh' and yfk is not null and yfk !=0 order by id";//查询语句	;
					$result = mysql_query($sql,$link);
					
					if (!mysql_num_rows($result)){
					 echo "<tr align='center'>";
					echo "<td  colspan='18' >表中无收票记录存在!</td>";
					// return false;					
					}	
					$i=0;
			while(@$row = mysql_fetch_assoc($result)){
				...
				echo "<td>{$row['kprq']}</td>";
				echo "<td>{$row['bz']}</td>";
				echo "<td  width='40px'><div style='/* text-align:center; */height:auto;' class='datagrid-cell datagrid-cell-c8-edit'><img class='GridTopImg' title='点击编辑修改记录' onclick='ShowEdit({$row['id']})' src='images/Edit.png'>|<img title='点击删除该条记录' class='GridTopImg' onclick='ShowDel({$row['id']})' src='images/DelBtn.png'></div></td>";
				echo "</tr>";
			}
			$sql2="select count(*) as rows, sum(yfk) as zyfk,sum(ysk) as zysk from bill where htbh='$htbh' and yfk is not null and yfk !=0";
			...
			echo "<tr align='center'>";
			echo "<td>合计</td>";
			echo "<td ><input  id='zyfk' placeholder=''  value='$zyf' type='text' readonly style='border:0;width: 100%;height: 60%;color:black;text-align: center;'></td>";  //总收票
			echo "<td ><input  id='zysk' placeholder=''  value='$zys' type='text' readonly style='border:0;width: 100%;height: 60%;color:black;text-align: center;'></td>";  //总税额
			echo "</tr>";
			echo "<tr align='center'>";
			echo "<td colspan='7' style='border:0;'> <a href='javascript:;' id='add' onclick='add()' title='添加收票记录' class='easyui-linkbutton l-btn l-btn-small l-btn-plain' iconcls='icon-add' plain='true' group=''><span class='l-btn-left l-btn-icon-left'><span class='l-btn-text'></span><span class='l-btn-icon icon-add'>&nbsp;</span></span></a>";
			...
			<form method='get' action='bill_luruphp_spend_invoice.php' id='Form'>
				<?php
				...
				<tr align='center' id="one">
					<td style="border: 0;">第<input style="border:0;width: 16px;text-align: center;" type='text' name="number" value="<?php echo $rows;?>">次收票</td>
					<td style="border: 0;"><textarea  name='yfk' id='yfk' style="width: 138px;height: 26px"placeholder='收票金额'></textarea></td>
					<td style="border: 0;"><textarea  name='ysk' id='ysk' style="width: 138px;height: 26px"placeholder='税额'></textarea></td>
					...
					<td style="border: 0;"><span class="ace-calendar-picker" >
									<div style="top: 40px;">
										<input  id="dpStart" placeholder="收票时间" name="kprq" autocomplete='off' type="text" class="ace-calendar-picker-input ace-input"style="width:100px;height: 60%;color:black">
					...
					<td style="border: 0;"><!-- <a href="javascript:;" id="add1" onclick="add1()"     title="添加收票记录" class="easyui-linkbutton l-btn l-btn-small l-btn-plain" iconcls="icon-add" plain="true" group=""><span class="l-btn-left l-btn-icon-left"><span class="l-btn-text"></span><span class="l-btn-icon icon-add">&nbsp;</span></span></a> --><a href="javascript:;" id="delete1" title="点击删除选定记录" onclick="delete1()" class="easyui-linkbutton l-btn l-btn-small l-btn-plain" iconcls="icon-remove" plain="true" group="" style="display: inline-block;"><span class="l-btn-left l-btn-icon-left"><span class="l-btn-text"></span><span class="l-btn-icon icon-remove">&nbsp;</span></span></a></td>
				
				</tr>
				<tr>
				<td  colspan='6' style='text-align: center;border: 0;'><center><a href='javascript:;' id='ok' class='easyui-linkbutton l-btn l-btn-small l-btn-plain' onclick='subForm();' ><span class='l-btn-left l-btn-icon-left'><span class='l-btn-text'>添加收票记录</span><span class='l-btn-icon icon-ok'>&nbsp;</span></span></a></td>
				</tr>
			...
			<!-- 			<div class="box-footer layer-footer" style="float:right"> -->
					...
					function Save() {
						...
						if(zysk!=''){
						$("#se" , window.parent.document).val(zysk.substring(0,zysk.length-1)); // 传给父窗口
						// $("#ysp" , window.parent.document).val(zyfk.substring(0,zyfk.length-1)); // 传给父窗口
						// $("#yfk" , window.parent.document).keyup(); //触发yfk的keyup事件更新应收款
						}else{
							$("#se" , window.parent.document).val(0); // 传给父窗口
							// $("#ysp" , window.parent.document).val(0); // 传给父窗口
							// $("#yfk" , window.parent.document).keyup(); //触发yfk的keyup事件更新应收款
						}
		...
		<script>
			function ShowEdit(id){
				var iid = id;
				 window.location.href="bill_change_spend_invoice.php?user=<?php echo $user;?>&id=" + iid + "";
				}
		</script>
		<script>
				function ShowDel(id) {					
						layer.open({
						 ...
						 yes: function () {
						                     var iid = id;
						                     window.location.href="bill_delete_spend_invoice.php?user=<?php echo $user;?>&id=" + iid + "";
						                 },
						});
					}		
			</script>
