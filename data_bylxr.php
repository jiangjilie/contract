...
// header("content-Type: text/html; charset=utf-8");//字符编码设置  
				...
				$sql="select id,time,nd,yf,lxr,xmmc,xmbh, sum(srhtze) as sz,sum(ysk) as ys,sum(gsk) as gs,sum(srse) as srse,sum(zchtze) as zz,sum(yfk) as yfk,sum(wfk) as wf,sum(zcse) as zcse,sum(fkje) as fkje,sum(lxse) as lxse from contract $where   group by lxr order by lxr desc";
				...
				$result = mysql_query($sql);
					while($row = mysql_fetch_assoc($result)){
						...
						$bj=($bj)."%";
							//将查询结构集重新数组化
							$data[]=array("id"=>$row["id"],"time"=>$row["time"],"nd"=>$row["nd"],"yf"=>$row["yf"],"lxr"=>$row["lxr"],"xmbh"=>$row["xmbh"],"xmmc"=>$row["xmmc"],"srze"=>$sz,"ysk"=>$ys,"srbl"=>$srbl,"zcze"=>$zz,"yzc"=>$yfk,"zcbl"=>$zcbl,"mlr"=>$ml,"blbj"=>$bj,"srse"=>$srse,"zlse"=>$zlse);
							// $data[] = $row;
						}
