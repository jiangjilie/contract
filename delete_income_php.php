	...
	<body>
			...
			//选择数据库
				...
				if($htbh=='' and  $xmbh=='' and $xmmc=='' and $dwmc=='' and $htmc=='' and $nd=='' and $yf==''){
						//从contract 中删除
						$del = "delete from contract where ishtlx='1' ";
						mysql_query($del);
						//从invoice中删除
						$del = "delete from invoice where ishtlx='1' ";
						mysql_query($del);
						//从bill中删除
						$del = "delete from bill where ishtlx='1' ";
						mysql_query($del);
						//从income中删除
						$del = "delete from income  ";
						$res_del =mysql_query($del);
						if($res_del){
							echo "<script>
							 window.location.href='income.php?user=$user';
							 </script>";
						}
				}
				...
				//从contract 中删除
				$del = "delete from contract $where and ishtlx = '1' ";
				mysql_query($del);
				//从invoice中删除
				$del = "delete from invoice $where and ishtlx = '1' ";
				mysql_query($del);
				//从bill中删除
				$del = "delete from bill $where and ishtlx = '1' ";
				mysql_query($del);
			//从income 中删除
				$del = "delete from income $where";
				$res_del = mysql_query($del);
				if($res_del){
					echo "<script>
					 window.location.href='income.php?user=$user';
