...
/**
        ...
        foreach($arr as $v){
            ...
            $objPHPExcel->getActiveSheet()
              ...
              ->setCellValue('G'.$key, $v['htze'])       
                ->setCellValue('H'.$key, $v['je'])
                ->setCellValue('I'.$key, $v['pzh'])
                ->setCellValue('J'.$key,$v['kprq'])
 ...
 $finish=str_replace("-",".",$finish);
  $wherelist = array();//获取查询条件
  	if(!empty($_GET['htbh'])){
  		$wherelist[] = "administrative.htbh like '%{$_GET['htbh']}%'";
  	}
  	if(!empty($_GET['xmbh'])){
  		$wherelist[] = "administrative.xmbh like '%{$_GET['xmbh']}%'";
  	}
  	if(!empty($_GET['xmmc'])){
  		$wherelist[] = "administrative.xmmc like '%{$_GET['xmmc']}%'";
  	}
  	if(!empty($_GET['dwmc'])){
  		$wherelist[] = "administrative.dwmc like '%{$_GET['dwmc']}%'";
  	}
  	if(!empty($_GET['htmc'])){
  		$wherelist[] = "administrative.htmc like '%{$_GET['htmc']}%'";
  	}
  	if(!empty($_GET['fkpz'])){
  		$wherelist[] = "administrative.fkpz like '%{$_GET['fkpz']}%'";
  	}
  	if(!empty($_GET['nd'])){
  		$wherelist[] = "administrative.nd like '%{$_GET['nd']}%'";
  	}
  	if(!empty($_GET['begin'])){
  ...
  if(count($wherelist) > 0){         //组装查询条件
	...
	$sql = "SELECT administrative.*,bill.jshj as je,bill.pzh,bill.kprq,bill.number FROM administrative left  JOIN bill on administrative.htbh=bill.htbh $where and bill.jshj is not null order by bill.htbh ";
  }else{
	  $sql = "SELECT administrative.*,bill.jshj as je,bill.pzh,bill.kprq,bill.number FROM administrative left  JOIN bill on administrative.htbh=bill.htbh where bill.jshj is not null order by bill.htbh ";
  }
 //先获取数据
 // $sql = "SELECT administrative.*,bill.jshj as je,bill.pzh,bill.kprq,bill.number FROM administrative left  JOIN bill on administrative.htbh=bill.htbh $where order by bill.htbh ";
 $res = mysql_query($sql);
 $arr = array();
