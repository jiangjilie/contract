...
$id = $_GET['id'];	
 ...
 //先获取数据
  $arr = array();
 for($i=0;$i<$countp;$i++ ){
	 $sql = "SELECT administrative.*,bill.jshj as je,bill.pzh,bill.kprq,bill.number FROM administrative left  JOIN bill on administrative.htbh=bill.htbh  where administrative.id=$data[$i] and bill.jshj is not null and bill.jshj !=0 order by bill.htbh ";
	 $res = mysql_query($sql);
