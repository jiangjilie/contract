...
/**
    ...
    function expExcel($arr,$name,$hz){
        ...
        $objPHPExcel->getActiveSheet()
            ...
            ->setCellValue('N1', '已收款')
            ->setCellValue('O1', '应收款')
            ->setCellValue('P1', '收款时间')
            ->setCellValue('Q1', '收款次数')
            ->setCellValue('R1', '备注')
            //设置第一行为红色字体
        ...
        foreach($arr as $v){
            ...
            $objPHPExcel->getActiveSheet()
                ...
                ->setCellValue('N'.$key, $ysk)
                ->setCellValue('O'.$key, $gsk)
...
@$htbh = $_GET['htbh'];
 //先获取数据
 $sql = "SELECT income.*,bill.jshj as je,bill.pzh,bill.kprq,bill.number FROM income left  JOIN bill on income.htbh=bill.htbh where income.htbh='$htbh'";
 $res = mysql_query($sql);
 $arr = array();
