...
/**
 ...
 $finish=str_replace("-",".",$finish);
  $wherelist = array();//获取查询条件
  	if(!empty($_GET['htbh'])){
  		$wherelist[] = "income.htbh like '%{$_GET['htbh']}%'";
  	}
  	if(!empty($_GET['xmbh'])){
  		$wherelist[] = "income.xmbh like '%{$_GET['xmbh']}%'";
  	}
  	if(!empty($_GET['xmmc'])){
  		$wherelist[] = "income.xmmc like '%{$_GET['xmmc']}%'";
  	}
  	if(!empty($_GET['dwmc'])){
  		$wherelist[] = "income.dwmc like '%{$_GET['dwmc']}%'";
  	}
  	if(!empty($_GET['htmc'])){
  		$wherelist[] = "income.htmc like '%{$_GET['htmc']}%'";
  	}
  	if(!empty($_GET['yf'])){
  		$wherelist[] = "income.yf like '%{$_GET['yf']}%'";
  	}
  	if(!empty($_GET['nd'])){
  		$wherelist[] = "income.nd like '%{$_GET['nd']}%'";
  	}
  	if(!empty($_GET['begin'])){
  ...
  if(count($wherelist) > 0){         //组装查询条件
	...
	$sql = "SELECT income.*,bill.jshj as je,bill.pzh,bill.kprq,bill.number FROM income left  JOIN bill on income.htbh=bill.htbh $where and bill.jshj is not null and bill.jshj !=0 order by bill.htbh ";
  }else{
	  $sql = "SELECT income.*,bill.jshj as je,bill.pzh,bill.kprq,bill.number FROM income left  JOIN bill on income.htbh=bill.htbh where bill.jshj is not null and bill.jshj !=0 order by bill.htbh ";
  }
 //先获取数据
 // $sql = "SELECT income.*,bill.jshj as je,bill.pzh,bill.kprq,bill.number FROM income left  JOIN bill on income.htbh=bill.htbh $where order by bill.htbh ";
 $res = mysql_query($sql);
 $arr = array();
