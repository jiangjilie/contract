...
$id = $_GET['id'];	
 ...
 //先获取数据
  $arr = array();
 for($i=0;$i<$countp;$i++ ){
	 $sql = "SELECT income.*,bill.jshj as je,bill.pzh,bill.kprq,bill.number FROM income left  JOIN bill on income.htbh=bill.htbh  where income.id=$data[$i] and bill.jshj is not null and bill.jshj !=0 order by bill.htbh ";
	 $res = mysql_query($sql);
