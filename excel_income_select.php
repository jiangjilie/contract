...
/**
    ...
    function expExcel($arr,$name){
        ...
        foreach($arr as $v){
            ...
            $objPHPExcel->getActiveSheet()
                ...
                ->setCellValue('N'.$key, $v['ysk'])
                ->setCellValue('O'.$key, $v['gsk'])
                ->setCellValue('P'.$key, $v['ykp'])
                ->setCellValue('Q'.$key, $v['kpmx'])
                ->setCellValue('R'.$key, $v['dept'])
                ->setCellValue('S'.$key, $v['bz']);
 ...
 //先获取数据
  $arr = array();
 for($i=0;$i<$countp;$i++ ){
	 $sql = "select * from income where id=$data[$i]";
	 $res = mysql_query($sql);
