...
/**
    ...
    function expExcel($arr,$name){
        ...
        $objPHPExcel->getActiveSheet()
            ...
            ->setCellValue('C1', '类别')
			->setCellValue('D1', '凭证号')
            ->setCellValue('E1', '开票日期')
            ->setCellValue('F1', '分管领导')
            ->setCellValue('G1', '申请人')
            ->setCellValue('H1', '发票种类')
            ->setCellValue('I1', '税率')
            ->setCellValue('J1', '发票号码')
            ->setCellValue('K1', '开票单位')
            ->setCellValue('L1', '开票内容')
            ->setCellValue('M1', '发票内容')
            ->setCellValue('N1', '金额')
            ->setCellValue('O1', '税额')
            ->setCellValue('P1', '价税合计')
            ->setCellValue('Q1', '备注')
            ->setCellValue('R1', '合同编号')
            ->setCellValue('S1', '项目编号')
            //设置第一行为红色字体
        ...
        foreach($arr as $v){
            ...
            $objPHPExcel->getActiveSheet()
                ...
                ->setCellValue('C'.$key, $v['lb'])
                ->setCellValue('D'.$key, $v['pzh'])
                ->setCellValue('E'.$key, $v['kprq'])
                ->setCellValue('F'.$key, $v['fgld'])
                ->setCellValue('G'.$key, $v['sqr'])
                ->setCellValue('H'.$key, $v['fpzl'])
                ->setCellValue('I'.$key, $v['sl'])
                ->setCellValue('J'.$key, $v['fphm'])
                ->setCellValue('K'.$key, $v['kpdw'])
                ->setCellValue('L'.$key, $v['kpnr'])
                ->setCellValue('M'.$key, $v['fpnr'])
                ->setCellValue('N'.$key, $v['je'])
                ->setCellValue('O'.$key, $v['se'])
                ->setCellValue('P'.$key, $v['jshj'])
                ->setCellValue('Q'.$key, $v['bz'])
                ->setCellValue('R'.$key, $v['htbh'])
                ->setCellValue('S'.$key, $v['xmbh']);
 ...
 //先获取数据
  $arr = array();
 for($i=0;$i<$countp;$i++ ){
	 $sql = "select * from invoice where id=$data[$i]";
	 $res = mysql_query($sql);
