<?php
    ...
    // header("Content-Disposition:filename=data.xls");// 文件名自己改，默认data.xls
		...
		header("Content-Disposition: attachment;filename=".'年度统计表'.".xls");    
   ...
   mysql_query("set names 'utf8'");
	...
	@$sql = "select nd,count(0) as total,count(ishtlx = '1' or null) as income,count(ishtlx = '2' or null) as spend,sum(srhtze) as sz,sum(ysk) as ys,sum(gsk) as gs,sum(zchtze) as zz,sum(yfk) as yfk,sum(wfk) as wf from contract  $where group by nd order by nd ";//查询语句	;
