...
/**
    ...
    function expExcel($arr,$name){
        ...
        foreach($arr as $v){
            ...
            $objPHPExcel->getActiveSheet()
                ...
                ->setCellValue('B'.$key, $v['xmbh'])
                ->setCellValue('C'.$key, $v['xmmc'])
                ->setCellValue('D'.$key, $v['dwmc'])
                ->setCellValue('E'.$key, $v['fkje'])
                ->setCellValue('F'.$key, $v['se'])
                ->setCellValue('G'.$key, $v['fksj'])
                ->setCellValue('H'.$key, $v['pzh'])
                ->setCellValue('I'.$key, $v['zy'])
                ->setCellValue('J'.$key, $v['bz']);
 ...
 //先获取数据
 $sql = "select * from purchase $where order by xmbh";
 $res = mysql_query($sql);
 $arr = array();
