...
/**
    ...
    function expExcel($arr,$name,$hz){
        ...
        $objPHPExcel->getActiveSheet()
            ...
            ->setCellValue('N1', '已付款')
            ->setCellValue('O1', '未付款')
            ->setCellValue('P1', '付款时间')
            ->setCellValue('Q1', '付款次数')
            ->setCellValue('R1', '备注')
            //设置第一行为红色字体
        ...
        foreach($arr as $v){
            ...
            $objPHPExcel->getActiveSheet()
                ...
                ->setCellValue('N'.$key, $yfk)
                ->setCellValue('O'.$key, $wfk)
                ->setCellValue('P'.$key,$v['kprq'])
                ->setCellValue('Q'.$key, $number)
                ->setCellValue('R'.$key, $v['bz']);
				$hz=$hz-$yfk;  //合同剩余部分；
				$i++;
        }
        //设置当前的表格
...
@$htbh = $_GET['htbh'];
 //先获取数据
 $sql = "SELECT spend.*,bill.jshj as je,bill.pzh,bill.kprq,bill.number FROM spend left  JOIN bill on spend.htbh=bill.htbh where spend.htbh='$htbh' and bill.jshj is not null";
 $res = mysql_query($sql);
 $arr = array();
 ...
 while ($row = mysql_fetch_assoc($res)){
  ...
  $sql = "SELECT htze FROM spend  where htbh='$htbh'";
  $res = mysql_query($sql);
