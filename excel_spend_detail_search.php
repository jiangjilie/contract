...
/**
    ...
    function expExcel($arr,$name){
        ...
        $objPHPExcel->getActiveSheet()
           ...
           ->setCellValue('C1', '项目编号')
           ->setCellValue('D1', '项目名称')
           ->setCellValue('E1', '乙方单位')
           ->setCellValue('F1', '合同名称')
           ->setCellValue('G1', '合同类型')
           ->setCellValue('H1', '联系人')
           ->setCellValue('I1', '签订时间')
           ->setCellValue('J1', '合同工期')
           ->setCellValue('K1', '付款方式')
           ->setCellValue('L1', '结算方式')
           ->setCellValue('M1', '合同总额')
           ->setCellValue('N1', '已付款')
        ...
        foreach($arr as $v){
            ...
            $objPHPExcel->getActiveSheet()
              ...
              ->setCellValue('C'.$key, $v['xmbh'])
              ->setCellValue('D'.$key, $v['xmmc'])
              ->setCellValue('E'.$key, $v['dwmc'])
              ->setCellValue('F'.$key, $v['htmc'])
              ->setCellValue('G'.$key, $v['htlx'])
              ->setCellValue('H'.$key, $v['lxr'])
              ->setCellValue('I'.$key, $v['qdsj'])
              ->setCellValue('J'.$key, $v['htgq'])
              ->setCellValue('K'.$key, $v['fkfs'])
              ->setCellValue('L'.$key, $v['jsfs'])
              ->setCellValue('M'.$key, $v['htze'])
              ->setCellValue('N'.$key, $v['je'])
              ->setCellValue('O'.$key,$v['kprq'])
              ->setCellValue('P'.$key, $v['pzh'])
 ...
 $finish=str_replace("-",".",$finish);
  $wherelist = array();//获取查询条件
  	if(!empty($_GET['htbh'])){
  		$wherelist[] = "spend.htbh like '%{$_GET['htbh']}%'";
  	}
  	if(!empty($_GET['xmbh'])){
  		$wherelist[] = "spend.xmbh like '%{$_GET['xmbh']}%'";
  	}
  	if(!empty($_GET['xmmc'])){
  		$wherelist[] = "spend.xmmc like '%{$_GET['xmmc']}%'";
  	}
  	if(!empty($_GET['dwmc'])){
  		$wherelist[] = "spend.dwmc like '%{$_GET['dwmc']}%'";
  	}
  	if(!empty($_GET['htmc'])){
  		$wherelist[] = "spend.htmc like '%{$_GET['htmc']}%'";
  	}
  	if(!empty($_GET['yf'])){
  		$wherelist[] = "spend.yf like '%{$_GET['yf']}%'";
  	}
  	if(!empty($_GET['nd'])){
  		$wherelist[] = "spend.nd like '%{$_GET['nd']}%'";
  	}
  	if(!empty($_GET['begin'])){
  		$wherelist[] = "bill.kprq >= '{$begin}'";
  	}
  	if(!empty($_GET['finish'])){
  		$wherelist[] = "bill.kprq <= '{$finish}'";
  	}
  	
  if(count($wherelist) > 0){         //组装查询条件
  	$where = " where ".implode(' AND ' , $wherelist); 
	$sql = "SELECT spend.*,bill.jshj as je,bill.yfk as billyfk,bill.pzh,bill.kprq,bill.number FROM spend left  JOIN bill on spend.htbh=bill.htbh $where and bill.jshj is not null and bill.jshj !=0 order by bill.htbh ";
  }else{
	  $sql = "SELECT spend.*,bill.jshj as je,bill.yfk as billyfk,bill.pzh,bill.kprq,bill.number FROM spend left  JOIN bill on spend.htbh=bill.htbh where bill.jshj is not null and bill.jshj !=0 order by bill.htbh ";
  }
 //先获取数据
 // $sql = "SELECT spend.*,bill.jshj as je,bill.yfk as billyfk,bill.pzh,bill.kprq,bill.number FROM spend left  JOIN bill on spend.htbh=bill.htbh $where order by bill.htbh ";
 $res = mysql_query($sql);
 $arr = array();
