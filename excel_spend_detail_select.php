...
$id = $_GET['id'];	
 ...
 //先获取数据
  $arr = array();
 for($i=0;$i<$countp;$i++ ){
	 $sql = "SELECT spend.*,bill.jshj as je,bill.pzh,bill.kprq,bill.number FROM spend left  JOIN bill on spend.htbh=bill.htbh  where spend.id=$data[$i] and bill.jshj is not null and bill.jshj !=0 order by bill.htbh ";
	 $res = mysql_query($sql);
