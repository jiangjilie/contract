    ...
    function expExcel($arr,$name){
        ...
        foreach($arr as $v){
            ...
            $objPHPExcel->getActiveSheet()
               ...
               ->setCellValue('N'.$key, $v['yfk'])
               ->setCellValue('O'.$key, $v['wfk'])
               ->setCellValue('P'.$key, $v['ysp']) 
               ->setCellValue('Q'.$key, $v['bz'])
               ->setCellValue('R'.$key, $v['spmx'])
               ->setCellValue('S'.$key, $v['se']);
...
$id = $_GET['id'];	
 ...
 //先获取数据
  $arr = array();
 for($i=0;$i<$countp;$i++ ){
	 $sql = "select * from spend where id=$data[$i]";
	 $res = mysql_query($sql);
