	...
	<head>
			              ...
			              <fieldset>
			                  <div class="form-group">
			                      <div  style="float: right;">
									  <button type="button"  style="background-color: #428BCA !important;border-color: #428BCA;border: 5px solid #FF" class="btn btn-primary" id="button";"><span class="fa fa-search" style="border: 5px solid #FF"></span>&nbsp;预览</button>		
									 <a href="javascript:;" id="export" title="批量导出数据" onclick="excel()" class=" l-btn l-btn-small l-btn-plain"  group=""><span class="l-btn-left l-btn-icon-left"><span class="l-btn-text">导出</span><span class="l-btn-icon icon-export">&nbsp;</span></span></a>									
								  </div>			                  							  							 							  
							  </div>
							  </div>
			              </fieldset>				
		...
		$("#button").click(function(){
			 $('#tb_user').bootstrapTable('destroy'); //动态加载表格之前，先销毁表格
				...
				if(xmmc==''&&xmbh==''&&dwmc==''&&htmc==''&&htbh==''&&nd==''&&yf==''&&begin==''&&finish==''){
					return false;
				}
		...
		<script>
			$('tb_user').colResizable({resizeMode:'overflow'});
		</script>
				<script>
				//修改
					function ShowEdit(id,ysk,ykp){
						layer.open({
							 ...
							 closeBtn: 1,
							 anim: 2,
							 content: "income_change.php?id=" + id + "&ysk=" + ysk + "&ykp=" + ykp,
							  // btn: ["", "取消"],
			...
			<script>
				function excel(){
					...
					var begin = $("#begin").val();
					var finish = $("#finish").val();
					// if(xmmc==''&&xmbh==''&&dwmc==''&&htmc==''&&htbh==''&&nd==''&&yf==''&&begin=='')
