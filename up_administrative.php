...
<html>
 ...
 <div id="tb" style="padding:5px;height:auto;background-color:#438eb9;color: white;" class="datagrid-toolbar">
 	<div id="divtoolbar" style="margin-bottom:5px">
		<form action="up_administrative.php" method="post" enctype="multipart/form-data" id='Form'>
		...
		<center>步骤一：请严格将Excel按如图顺序排列顺序,<span style="color: red;">注意A,B,C,······,L 各列的顺序</span>，有效数据是从<span style="color: red;">第A列第2行</span>开始读取<a href='excel_administrative.php?nd=99' style='margin-left:30px'>模板下载</a></center>
		</div>
		<img title=''  style=""src='images/administrative.png'>	
		</div>
		<div style="margin: 8px;padding:2px;border:2px;background-color: ;">
	...
	<script>
		...
		layer.open({
			 ...
			 anim: 2,
			 shadeClose: true,
			 content: '<center><h4 style="padding:5px5px;magin:2px;">确定将数据导入到<span style="color:red;">行政合同表</span>吗</h4></center>',
			 btn: ["确定", "取消"],
			 yes: function () {
			                    $("#Form").submit();
			                 },
...
if (copy($tmp, $filename)) {
 ...
 require_once 'PHPExcel/Reader/Excel5.php';
  require_once 'PHPExcel/Shared/Date.php';
   ...
   $sql = "insert into administrative (nd,htbh,htmc,dwmc,qdsj,lxr,htze,yfk,wfk,fkpz,bz) values ('{$strs[0]}','{$strs[1]}','{$strs[2]}','{$strs[3]}','$time','{$strs[5]}','{$strs[6]}','{$strs[7]}','{$strs[8]}','{$strs[9]}','{$strs[10]}')";
...
<script>
 ...
 title: '提示'
   ...
   // $sql2 = "insert into contract (nd,yf,xmbh,xmmc,dwmc,htmc,lxr,fkje,bz,ishtlx) values ('{$strs[0]}','{$strs[1]}','{$strs[2]}','{$strs[3]}','{$strs[4]}','{$strs[5]}','{$strs[6]}','{$strs[7]}','{$strs[10]}','3')";
