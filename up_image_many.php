...
<html>
	...
	<form action="" method="post" enctype="multipart/form-data" id='Form'>
		       <table>
				...
				<!-- 	<td>
						...
						<a href="javascript:;" title="上传单个文件夹" onclick="up_image_one()" class="easyui-linkbutton l-btn l-btn-small l-btn-plain" iconcls="icon-import" plain="true" group=""><span class="l-btn-left l-btn-icon-left"><span class="l-btn-text">批量上传文件夹</span><span class="l-btn-icon icon-import">&nbsp;</span></span></a>
					</td> -->
	...
	$count = count($file);//计算file数组长度
	$tmp_name=$_FILES['file']['tmp_name']; //上传成功后在临时文件夹的文件名字
	$paths=$_POST['path'];
	$countp=count($path);//计算path数组长度
	//多文件夹上传
	for($i=0;$i<$countp;$i++ ){
		$path=$paths[$i];
	...
	// for($i=0;$i<$count;$i++ ){
	// 	$path=$_POST['path'][$i];
	// 	$path=substr($path,0,strpos($path,'/'));   //substr() 截取字符，参数（字符串，开始位置，结束位置）,strpos() - 查找字符串首次出现的位置
	// 	if(!is_dir('image/'.$path)){ //需要先判断文件夹是否存	
	// 	mkdir('image/'.$path,0777);	 //mkdir 创建文件夹
	// 	}	
	// 	move_uploaded_file($tmp_name[$i],"./image/".$path."/".$file[$i]);
