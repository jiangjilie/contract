...
<html>
 ...
 <div id="tb" style="padding:5px;height:auto;background-color:#438eb9;color: white;" class="datagrid-toolbar">
 	<div id="divtoolbar" style="margin-bottom:5px">
		<form action="up_income.php" method="post" enctype="multipart/form-data" id='Form'>
		...
		<center>步骤一：请严格将Excel按如图顺序排列顺序,<span style="color: red;">注意A,B,C,······,Q 各列的顺序</span>，有效数据是从<span style="color: red;">第A列第二行</span>开始读取<a href='excel_income.php?nd=99' style='margin-left:30px'>模板下载</a></center>
		</div>
		<img title=''  style="width: 1200px;"src='images/income.png'>	
		</div>
		<div style="margin: 8px;padding:2px;border:2px;background-color: ;">
	...
	<script>
		...
		layer.open({
			 ...
			 anim: 2,
			 shadeClose: true,
			 content: '<center><h4 style="padding:5px5px;magin:2px;">确定将数据导入到<span style="color:red;">收入合同表</span>吗</h4></center>',
			 btn: ["确定", "取消"],
			 yes: function () {
			                    $("#Form").submit();
			                 },
...
if (copy($tmp, $filename)) {
 ...
 require_once 'PHPExcel/Reader/Excel5.php';
  ...
  require_once 'PHPExcel/Shared/Date.php';
   ...
   $sql = "insert into income (nd,htbh,xmbh,xmmc,dwmc,htmc,htlx,lxr,qdsj,htgq,fkfs,jsfs,htze,ysk,gsk,ykp,bz) values ('{$strs[0]}','{$strs[1]}','{$strs[2]}','{$strs[3]}','{$strs[4]}','{$strs[5]}','{$strs[6]}','{$strs[7]}','$time','{$strs[9]}','{$strs[10]}','{$strs[11]}','{$strs[12]}','{$strs[13]}','{$strs[14]}','{$strs[15]}','{$strs[16]}')";
...
<script>
 ...
 title: '提示'
   ...
   $sql2 = "insert into contract (nd,htbh,xmbh,xmmc,dwmc,htmc,lxr,qdsj,srhtze,ysk,gsk,bz,ishtlx) values ('{$strs[0]}','{$strs[1]}','{$strs[2]}','{$strs[3]}','{$strs[4]}','{$strs[5]}','{$strs[7]}','{$strs[8]}','{$strs[12]}','{$strs[13]}','{$strs[14]}','{$strs[16]}','1')";
   $result=mysql_query($sql2);
   $sql3="insert into bill (htbh,kprq,jshj,ishtlx) values('{$strs[1]}','{$strs[8]}','{$strs[13]}','1')";
   mysql_query($sql3);
   }
}
